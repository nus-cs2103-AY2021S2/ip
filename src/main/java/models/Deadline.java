...
public class Deadline extends Todo {
  ...
  private LocalDateTime parseStringToLocalDateTime(String deadlineString)
    ...
    return LocalDateTime.parse(
        deadlineString.length() == 15 ? deadlineString : String.format("0%s", deadlineString),
        inputDateFormat);
  }
   ...
   * 
   * @return String containing deadline of the Deadline object, parsed from LocalDateTime object in
   *         a prettier format
   */
  ...
  @Override
  /**
   * Method overridden the super class' to return a new Deadline that is marked as done instead of a
   * new Todo
   * 
