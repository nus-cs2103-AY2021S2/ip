...
public class Deadline extends Todo {
    ...
    private static final DateTimeFormatter inputDateFormat = DateTimeFormatter.ofPattern("dd/MM/yyyy HHmm");
    private static final DateTimeFormatter outputDateFormat = DateTimeFormatter.ofPattern("EEE dd MMM yyyy HH:mm");

    /**
     * Constructor to allow setting of the deadline in a Deadline object
     * 
     * @param message        String message that a Todo contains
     * @param deadlineString String specific to deadlines denoting when the deadline
     *                       is due
     */
     ...
     * @param deadlineString String specific to deadlineStrings denoting the
     *                       deadlineString of a Deadline Todo
     */
     ...
     */
    private LocalDateTime parseStringToLocalDateTime(String deadlineString) throws DateTimeParseException {
        return LocalDateTime.parse(
                deadlineString.length() == 15 ? deadlineString : String.format("0%s", deadlineString), inputDateFormat);
    }
    ...
    public String getDeadline() {
        return this.deadlineDateTime.format(inputDateFormat).toString();
    }

    /**
     * Getter method for deadline time from LocalDateTime of a Deadline object
     * 
     * @return String containing deadline of the Deadline object, parsed from
     *         LocalDateTime object in a prettier format
     */
    public String getPrettierDeadlineDateTime() {
        return String.format("%s hrs", this.deadlineDateTime.format(outputDateFormat).toString());
    }
