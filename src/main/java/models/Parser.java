...
public class Parser {
  ...
  /**
   * Initialises Command object by taking first item of the list as the command and the rest of the
   * itemms of the string gets taken as the command arguments Null safety is enforced through use of
   * Optionals
   */
  ...
  /**
   * Gets the first String passed in from the list of inputs, which is the Command in the input line
   * 
  ...
  public List<String> getCommandArgs() throws DukeCommandNotFoundException {
    return this.commandArgs.orElseThrow(
        () -> new DukeCommandNotFoundException("No command arguments were supplied from input."));
  }

  /**
   * Used for level 1 to get full argument passed into the CLI, where todo, events and deadline were
  ...
  public List<String> getFullCommand() throws DukeCommandNotFoundException {
    return this.fullCommand.orElseThrow(
        () -> new DukeCommandNotFoundException("No command arguments were supplied from input."));
  }
}
