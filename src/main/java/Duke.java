    ...
    private Duke process(String input) {
        ...
        if (input.equals("bye")) {
            ...
            try {
                ...
                switch (tokens[0]) {
                    case "todo":
                        t = new TaskTodo(input.substring("todo".length() + 1).trim(), false);
                        break;
                    case "deadline":
                        String deadline = input.substring(input.indexOf("/by ") + 4);
                        t = new TaskDeadline(input.substring("deadline".length() + 1, input.indexOf(" /by ")).trim(),
                                false, deadline);
                        break;
                    case "event":
                        String time = input.substring(input.indexOf("/at ") + 4);
                        t = new TaskEvent(input.substring("event".length() + 1, input.indexOf(" /at ")).trim(),
                                false, time);
                        break;
                }
                newStore.add(t);
                ...
                newAgent = new Duke("Invalid command. Are you missing some arguments?",
                        this.store,
                        this.done);
            }
            return newAgent;
            ...
            int index = 0;
            try {
                index = Integer.parseInt(tokens[1].trim());
                if (index < 1 || index > this.store.size()) {
                    throw new DukeException("Index " + index + " out of bounds for list of size " + this.store.size());
                }
                List<Task> newStore = new ArrayList<>();
                String response = "";
                Task t;
                ...
                newAgent = new Duke("Please enter an integer", this.store, false);
            } catch (DukeException dukeException) {
                newAgent = new Duke(
                        "Index " + index + " out of bounds for list of size " + this.store.size(),
                        this.store, false);
            }
        } else {
