...
public class Deadline extends Task {
     ...
     */
    public static Task createDeadline(String[] command) throws RonaldException {

        assert command != null : "Deadline commands cannot be null";

        if (command.length == 1) {
            throw new RonaldException("There's not enough information about your Deadline order!");
        }

        String[] args = command[1].split(" /by ", 2);

        if (args.length == 1 || args[0].isEmpty() || args[1].isEmpty()) {
            throw new RonaldException("Looks like your Deadline order isn't complete...");
        }
    ...
    public String toString() {
        return "[D]" + super.toString() + " (by: " + dateTime.getDateTimeForDisplay() + "\n";
    }
}
