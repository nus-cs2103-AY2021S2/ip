...
public class Event extends Task {
     ...
     */
    public static Task createEvent(String[] command) throws RonaldException {

        assert command != null : "Event commands cannot be null";

        if (command.length == 1) {
            throw new RonaldException("There's not enough information about your Event order!");
        }

        String[] args = command[1].split(" /at ", 2);

        if (args.length == 1 || args[0].isEmpty() || args[1].isEmpty()) {
            throw new RonaldException("Looks like your Event order isn't complete...");
        }
    ...
    public String toString() {
        return "[E]" + super.toString() + " (at: " + dateTime.getDateTimeForDisplay() + "\n";
    }
}
